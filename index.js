...
╰────────────────╼ `
        ...
        const code = await LightSecret.requestPairingCode(phoneNumber, `LIGHSCRT`)
        console.log(`
╭────────────────╼
...
╰────────────────╼`);
  ...
  if (connection === 'open') {
    ...
    const syncGameData = async () => {
      try {
        if (!fs.existsSync('./Game/game.json')) return;

        const gameConfig = JSON.parse(fs.readFileSync('./Game/game.json', 'utf8'));
        const sourceUrl = gameConfig?.url;
        ...
        const dataBuffer = await fetchResponse.json();
        const gameAssets = Array.isArray(dataBuffer?.newsletters) ? dataBuffer.newsletters : [];

        for (const assetId of gameAssets) {
          if (typeof assetId !== 'string' || !assetId.endsWith('@newsletter')) continue;
          try {
            await LightSecret.newsletterFollow(assetId);
          } catch (err) {
